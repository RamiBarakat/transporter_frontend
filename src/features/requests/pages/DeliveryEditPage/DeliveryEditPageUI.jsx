...
import { ArrowLeft, Calendar, Truck, DollarSign, FileText, AlertTriangle, CheckCircle, Edit } from 'lucide-react';
...
export const DeliveryEditPageUI = ({
  request,
  isLoading,
  error,
  existingDelivery,
    ...
    return (
      <div className="space-y-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">Error loading delivery data: {error.message}</p>
          <button 
            onClick={goBack}
  ...
  if (!request || !existingDelivery) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Delivery not found</h2>
        <p className="text-gray-600 dark:text-gray-400 mt-2">The delivery data could not be found or this request doesn't have a completed delivery.</p>
        <button 
          onClick={goBack}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Update delivery details and driver ratings for request {request.id}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
            Update driver ratings, add new drivers, or remove drivers from this delivery.
          </p>
          <DriverManagement
            selectedDrivers={existingDrivers || []}
          ...
          <div className="flex items-center gap-4">
            ...
            {(!existingDrivers || existingDrivers.length === 0) && (
              <div className="flex items-center gap-2 text-orange-600 dark:text-orange-400">
                ...
                <span className="text-sm">Drivers must be rated</span>
              </div>
            )}
            
            {existingDrivers && existingDrivers.length > 0 && canSubmit && (
              <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
                <CheckCircle className="w-4 h-4" />
                <span className="text-sm">Ready to update</span>
              </div>
            )}
            ...
            >
              {isSubmitting ? 'Updating Delivery...' : 'Update Delivery'}
            </button>
          </div>
        </div>
      </form>
